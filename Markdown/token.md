...
4. Model Config (save as configs/alpacore-1.3b.json)
...
tokd ds.map(chunk, batched-False, remove_columns=["text"]).train_t est_split(0.01, seed=42)

collator = DataCollator For LanguageModeling( tok, mlm=False) args = TrainingArguments( output_dir="out/alpacore-1.3b",

per_device_train_batch_size=2,

gradient_accumulation_steps=16,

learning_rate=2e-4, warmup_steps=2000, max_steps=100000, 1r_scheduler_type="cosine", bf16=True, logging_steps=50, save_steps=1000, save_total_limit=3, report_to="none"
...
6. Sculpt Persona (SFT)

Prepare data/sft/alpaca_persona.jsonl with lines like:
...
8. Export to GGUF (for Termux/Android)
